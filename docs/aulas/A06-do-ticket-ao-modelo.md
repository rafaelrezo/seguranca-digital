# A06 — Como antecipar o que pode dar errado?

A04 tornou visível uma requisição de cesta e a representou como `pessoa → navegador → serviço → dado`. A05 mostrou que sessão válida e autorização sobre o recurso são decisões diferentes. Agora usamos esse mapa para procurar ameaças sistematicamente. Recupere os registros disponíveis; se os resultados de execução não estiverem preservados, use as regras esperadas sem tratá-las como fatos testados.
...
Abra o diagrama da A04 e a tabela da A05, se disponíveis. Separe **regra esperada** de **resultado observado**. Crie um documento `A06-Cestas` para acompanhar o modelo. Use somente dados fictícios; não é necessário iniciar o Juice Shop nem executar testes nesta oficina.

Se faltar a entrada anterior, use este cenário fornecido: Ana e Bruno possuem cestas distintas; o navegador envia uma requisição com contexto de sessão e identificador; o servidor deve validar sessão e propriedade antes de responder. O conteúdo das cestas é um ativo e o registro de decisão precisa permitir revisão sem guardar o segredo. A implementação interna e os resultados dos testes permanecem desconhecidos.

!!! question "Checkpoint — por que quatro casos não cobrem o sistema?"
    Escreva uma pergunta sobre o fluxo que a tabela de sessão/propriedade não responde. Ela deve indicar o que precisa continuar protegido. Preserve essa pergunta: o modelo e as lentes abaixo devem ajudá-lo a torná-la verificável.
...
O desenho retoma as responsabilidades da A04. Acrescente somente elementos que ajudem a localizar a pergunta registrada no checkpoint.
...
flowchart LR
    ...
    B -->|GET /rest/basket/A + contexto| API((API — processo))
...
!!! question "Checkpoint — a caixa é um ativo ou uma responsabilidade?"
    No esboço, localize quem solicita, quem decide e onde o estado persiste. Rotule as mensagens e marque a fronteira navegador–servidor. Explique qual perda de valor pode ocorrer em um desses pontos. Pare se não conseguir seguir o uso legítimo antes de acrescentar ameaças.
...
### Registro, revisão e entrega

**Missão:** entregar ao revisor um modelo mínimo e três ameaças verificáveis. **Tempo de referência:** 90 minutos, incluindo explicação e oficina. **Trabalho:** dupla, condutor e revisor alternados. O procedimento de ferramenta está na seção anterior; a tabela abaixo acompanha os resultados.

| Ponto do modelo | Origem da candidata e propriedade | Condição, caminho, ativo e consequência | Evidência que mudaria a conclusão | Estado e limite |
|---|---|---|---|---|
| preencher | ferramenta ou análise manual | preencher | preencher | preencher |

Registre três candidatas em ao menos dois pontos e uma descartada com justificativa. Uma sugestão genérica não é evidência de que o sistema foi analisado pela ferramenta. Não marque uma propriedade falsa apenas para obter sugestões.

Troque o diagrama e a tabela com outra dupla por seis minutos. O revisor deve indicar um ponto que não conseguiu localizar e uma alteração concreta. Faça a correção e registre antes/depois da formulação ou do desenho.

### Alternativa sem rede ou geração contextual

Desenhe em papel ou no editor disponível: pessoa, navegador, serviço, cestas e registro de decisão, com mensagens e fronteira. Use as perguntas STRIDE explicadas nesta página e estas **candidatas didáticas manuais**, sem atribuí-las ao gerador:

| Ponto | Pergunta para desenvolver | Condição que ainda precisa de evidência |
|---|---|---|
| resposta do serviço | conteúdo poderia chegar a outra pessoa? | vínculo entre sessão e proprietário |
| registro de decisão | seria possível negar autoria sem rastro suficiente? | existência e proteção dos campos de auditoria |
| atualização da cesta | um dado poderia ser alterado indevidamente? | autoridade exigida para a ação de alteração |
| pagamento por cartão | o recorte permite formular ameaça sobre cartão? | presença desse fluxo no escopo |

Escolha, adapte e descarte conforme o modelo. Para cada candidata, complete condição, caminho, consequência e evidência. Declare **análise manual**; a competência de operar o gerador permanece não demonstrada. Se o botão por contexto estiver ausente ou genérico, não improvise configurações: preserve a tabela e siga por essa alternativa.

### Entrega e rubrica

Se atribuída no Classroom, envie `A06-sobrenome1-sobrenome2.pdf`: diagrama, tabela, candidata descartada, modalidade utilizada, observação do revisor e correção. Preserve também o modelo editável local quando disponível.

| Critério | Concluído | Precisa revisar |
|---|---|---|
| Modelo | permite narrar o uso normal | componentes sem função ou mensagens |
| Candidatas | três ameaças completas, localizadas em dois pontos | categorias soltas |
| Proveniência | propriedade e origem da sugestão explícitas | análise manual atribuída à ferramenta |
| Evidência | dado proposto confirma ou enfraquece | coleta vaga |
| Revisão | correção identificável e descarte justificado | aceitação automática |

**Encerramento:** salve o arquivo local, feche e reabra para conferir que o modelo foi preservado; mantenha somente informações fictícias e sanitizadas. Não publique o modelo em repositório ou serviço remoto. **Extensão:** altere uma premissa do modelo e explique quais candidatas precisam ser revistas.

### Revisão rápida

1. Por que o DFD não demonstra a implementação da autorização?
2. Qual diferença existe entre geração por tipo e por contexto?
3. Que evidência faria você descartar uma candidata aceita inicialmente?
...
A07 recebe uma narrativa técnica consolidada e muda a pergunta: em vez de procurar novas ameaças ou escolher controles, a turma aprenderá a reconhecer **comportamentos observáveis** e a localizá-los no MITRE ATT&CK. O produto da A06 — modelo, ameaças e limites — permanece como referência, mas não será tratado como evidência de incidente. O cenário industrial será fornecido na própria A07; ele não foi observado no modelo de cestas.

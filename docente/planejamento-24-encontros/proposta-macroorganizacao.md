...
| **A09 — Quais riscos a organização aceita e quais precisa tratar?** | Critérios de consequência, plausibilidade e incerteza; avaliação comparada de riscos já formulados; alternativas de tratamento sob restrição; autoridade de aceitação, proprietário, residual estimado e revisão | Professor compara até dois riscos fornecidos, explicita critérios e restrições, confronta alternativas de tratamento e conduz a justificativa de prioridade e aceitação | Até dois riscos avaliados com critérios, prioridade, decisão de tratamento, proprietário e residual estimado. **Ponte:** Decisão de tratamento e critério de aceitação para selecionar e avaliar controles em A10 | 60/40 |
| **A10 — Como demonstrar que os controles atendem ao risco?** | Seleção e justificativa de controles; função da ISO/IEC 27002 e declaração de aplicabilidade no SGSI; política/padrão/procedimento; desenho, implantação e operação eficaz; amostra de evidência, exceção, desvio e melhoria | Professor transforma a decisão A09 em recorte de aplicabilidade e regra de manutenção; compara política escrita, configuração e registros de uso/revogação para julgar uma exceção sem evidência suficiente | Recorte de aplicabilidade, regra/procedimento e avaliação de controle com evidência, desvio e ação de melhoria. **Ponte:** Restrição de manutenção e limite da evidência para analisar consequência física em A11 | 50/50 |
...
|---|---|---|---:|
| P1 — governança e tratamento | apresentado A08; consolidado após A12 | 3–4 páginas: um fluxo, até dois riscos, uma decisão de tratamento, responsável, referência e teste; inclui a conclusão de modelagem | 2h |

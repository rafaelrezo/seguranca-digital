...
### O que muda em tecnologia operacional

Em TI, reiniciar, isolar ou atualizar pode ser uma resposta relativamente comum. Em OT, a mesma ação pode remover visibilidade, interromper controle, danificar produto ou levar o processo a um estado inseguro. A prioridade não é “manter tudo ligado” nem “isolar imediatamente”, mas coordenar segurança cibernética com operação, engenharia e safety.

Isso muda a pergunta de resposta: além de “o controle reduz propagação?”, é preciso perguntar “qual função será perdida, existe modo manual ou redundante, quem autoriza a transição e como confirmaremos o estado seguro?”. Reversibilidade e observabilidade são propriedades da intervenção, não garantias absolutas.

### Da linha do tempo à decisão

Uma linha do tempo defensável conserva quatro camadas separadas:

1. **evento registrado:** texto, origem e horário tal como coletados;
2. **interpretação:** significado técnico limitado daquele registro;
3. **hipótese:** explicação que conecta vários eventos;
4. **decisão:** ação proporcional e condição para revisá-la.

Misturar essas camadas produz certeza artificial. A ordem `login → timeout → stop` justifica investigar uma relação; não prova que o login causou a parada.
...
- [NIST SP 800-61 Rev. 2 — Computer Security Incident Handling Guide](https://csrc.nist.gov/pubs/sp/800/61/r2/final) — referência histórica para tratamento de incidentes; verificar revisões vigentes no catálogo do NIST.
- [NIST SP 800-92 — Guide to Computer Security Log Management](https://csrc.nist.gov/pubs/sp/800/92/final) — geração, proteção, retenção e análise de registros.

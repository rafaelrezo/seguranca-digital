...
### CWE, CVE, CVSS e exploração conhecida

Esses vocabulários respondem a perguntas diferentes:

- **CWE** organiza tipos de fraqueza e ajuda a explicar a classe da causa provável;
- **CVE** identifica publicamente uma vulnerabilidade específica em produto e versão;
- **CVSS** padroniza a descrição de severidade técnica, mas não substitui impacto do negócio, exposição real nem prioridade organizacional;
- **CISA KEV** relaciona vulnerabilidades CVE com evidência de exploração conhecida e orienta priorização operacional para organizações no escopo aplicável.

No caso `acquisitions.md`, descreva primeiro a condição observada. Só associe uma classificação quando seus critérios realmente coincidirem e nunca invente um CVE para a implantação local.
...
- [RFC 9110 — HTTP Semantics](https://www.rfc-editor.org/rfc/rfc9110) — significado de método, recurso, representação e código de status.
- [NIST National Vulnerability Database](https://nvd.nist.gov/) — enriquecimento de registros CVE e métricas; não é fonte para criar identificadores.
- [FIRST — CVSS v4.0](https://www.first.org/cvss/v4-0/) — especificação oficial da métrica de severidade.
- [CISA Known Exploited Vulnerabilities Catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) — vulnerabilidades com exploração conhecida.
